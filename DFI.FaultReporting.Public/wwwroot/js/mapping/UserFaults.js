//CODE SUMMARY:
//This file controls the user faults map. A user can search for locations on the map using the autocomplete search,
...
function initViewMap() {
    ...
    //Find map on page
    var faultMap = document.getElementById('userFaultsMap');

    if (faultMap != null) {
        //Create new map control
        map = new atlas.Map('userFaultsMap', {
        ...
        map.events.add('ready', function () {
            ...
            faults.forEach(function (fault) {
                ...
                if (!geoJsonFaults.map(e => e.properties.id).includes(geoJsonFault.properties.id)) {
...
function markerClicked(e) {
    ...
    //The clicked marker represents a single fault.
        ...
        //Create an href value for the "View details" link and add the ID of the fault as a query parameter. This is needed for linking reports to a fault on the "ReportDetails" page.
        var url = "/Faults/ReportDetails/?ID=" + marker.properties.id;
        ...
        popup.setOptions({
            content: `<div style="padding:10px;">
                              ...
                              <a id="linkReportDetails" href="${url}" class="btn btn-outline-primary btn-sm"><i class="fa-solid fa-magnifying-glass-plus"></i> View details</button>
                           </div>`,
            position: marker.getOptions().position,
            closeButton: true
        });
